 ...
 * CplVSPool class
 */
CplVSPool_DX8::CplVSPool_DX8(LPDIRECT3DDEVICE8 lpd3d)
	:CplVSPool(0), m_pd3dDevice(lpd3d)
{
}

CplVSPool_DX8::~CplVSPool_DX8()
{
}


HVS CplVSPool_DX8::create(DWORD handler)
{
	HVS hVS;
	...
	hVS->dwFormat = convert_to_DX8_format(handler);
	hVS->ulSize = 0;
	hVS->ulIndex = 0;
	hVS->pLocked = 0;
...
HVS CplVSPool_DX8::expand(HVS) 
{
	return 0;
}

BYTE* CplVSPool_DX8::lock(HVS hVS, ulong offset, ulong size)
{
	if ( hVS==0 || hVS->pLocked ) return 0;
	return hVS->pLocked;
}

bool CplVSPool_DX8::unlock(HVS hVS) 
{
...
bool CplVSPool_DX8::release(HVS hVS, DWORD usage) 
{
	del_entry(hVS);
	return false;
}

bool CplVSPool_DX8::freeup_entry(HVS hVS)
{
	if ( hVS==0 || hVS->pLocked==0 ) return true;
	return false;
}
