 ...
 * CplVBPool class
 */
CplVBPool_DX8::CplVBPool_DX8(LPDIRECT3DDEVICE8 lpd3d)
	:CplVBPool(0), m_pd3dDevice(lpd3d)
{
}

CplVBPool_DX8::~CplVBPool_DX8()
{
}


HVB CplVBPool_DX8::create(ulong length, WORD format, DWORD usage, const BYTE* buf)
{
	HVB hVB;
	...
	hVB->dwFormat = convert_to_DX8_format(format);
	hVB->ulSize = length*calculate_vertex_size(hVB->dwFormat);
	hVB->ulIndex = 0;
	hVB->pLocked = 0;
    // Create the vertex buffer.
    if( FAILED( m_pd3dDevice->CreateVertexBuffer( hVB->ulSize, usage, hVB->dwFormat, D3DPOOL_DEFAULT, &pVB ) ) )
		return 0;
...
HVB CplVBPool_DX8::expand(HVB) 
{
	return 0;
}

BYTE* CplVBPool_DX8::lock(HVB hVB, ulong offset, ulong size)
{
	LPDIRECT3DVERTEXBUFFER8 pVB;
	if ( hVB==0 || hVB->pLocked ) return 0;
	const ulong cb = calculate_vertex_size(hVB->dwFormat);
	pVB = (LPDIRECT3DVERTEXBUFFER8)hVB->hBase;
    if( FAILED( pVB->Lock( offset*cb, size*cb, (BYTE**)&(hVB->pLocked), 0 ) ) )
		return 0;
	return hVB->pLocked;
}

bool CplVBPool_DX8::unlock(HVB hVB) 
{
	LPDIRECT3DVERTEXBUFFER8 pVB;
...
bool CplVBPool_DX8::release(HVB hVB, DWORD usage) 
{
	del_entry(hVB);
	return false;
}

bool CplVBPool_DX8::freeup_entry(HVB hVB)
{
	LPDIRECT3DVERTEXBUFFER8 pVB;
	if ( hVB==0 || hVB->pLocked==0 ) return true;
	pVB = (LPDIRECT3DVERTEXBUFFER8)hVB->hBase;
